...
WMS_SERIAL_PORT="$(bashio::config 'wms_serial_port')"
...
WMS_SERIAL_PORT="${WMS_SERIAL_PORT:-/dev/ttyUSB0}"
LOG_LEVEL="${LOG_LEVEL:-info}"

export MQTT_SERVER
export MQTT_USER
export MQTT_PASSWORD
export WMS_SERIAL_PORT
export LOG_LEVEL
export WMS_KEY
export WMS_PAN_ID
export WMS_CHANNEL
...
echo "WMS_SERIAL_PORT=${WMS_SERIAL_PORT}"

if [ ! -e "${WMS_SERIAL_PORT}" ] ; then
  echo "Device ${WMS_SERIAL_PORT} not present. Waiting up to 30s..."
  for i in $(seq 1 30); do
    if [ -e "${WMS_SERIAL_PORT}" ]; then break; fi
    sleep 1
  done
fi
